...
##########################################################add index j###################################################
    def optimize_weight(self, inputs, ref, mask_in,model,j):
        ...
        dx = model.weight.grad
        print(dx.shape)
        ##################################################### update the loss value ####################################
        mask_test = torch.cat([torch.ones([j, dx.shape[1]]), torch.zeros([dx.shape[0] - j, dx.shape[1]])],
                               0)
        grad = dx * mask_test.cuda()
        loss_value = torch.sum(grad)
        loss_weight.set_(loss_value.cuda())
        ################################################################################################################
